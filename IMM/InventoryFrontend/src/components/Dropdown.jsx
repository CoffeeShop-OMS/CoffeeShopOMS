  ...
  return (
    <div ref={dropdownRef} className="relative w-full">
      <button
        ...
        className={`w-full border border-[#E2DDD8] rounded-xl px-3.5 py-2.5 text-sm text-[#2C1810] bg-white transition focus:outline-none focus:border-[#6B3E26] focus:ring-2 focus:ring-[#6B3E26]/10 flex items-center justify-between ${className}`}
      >
        <span className={displayValue ? 'text-[#2C1810]' : 'text-[#C4B8B0]'}>
          {displayValue || placeholder}
        </span>
        <ChevronDown
          className={`w-4 h-4 text-[#9E8A7A] transition-transform duration-200 ${
            isOpen ? 'rotate-180' : ''
          }`}
        />
      </button>
