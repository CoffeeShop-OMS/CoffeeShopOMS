      ...
      {isLoading ? (
        ...
        items.map((item) => {
          ...
          return (
            <div
              key={item.id}
              className={`px-4 py-4 ${item.isArchived ? 'bg-slate-50/80' : item.isOut ? 'bg-red-50/50' : ''}`}
            >
              ...
              <div className="flex items-center gap-3 mb-3 flex-wrap">
                ...
                <span className={`text-sm font-bold ${item.isArchived ? 'text-slate-500' : item.isOut ? 'text-red-500' : item.isLow ? 'text-amber-600' : 'text-[#1C100A]'}`}>
                  {item.stock}
                </span>
              </div>
